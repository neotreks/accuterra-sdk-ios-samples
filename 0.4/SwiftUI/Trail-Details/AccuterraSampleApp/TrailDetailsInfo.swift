...
struct TrailDetailsInfo {
            ...
            do {
                ...
                if let navigationInfo = trail?.navigationInfo {
                    ...
                    if let trailHead = navigationInfo.trailHead {
                        trailHeadValues.append(contentsOf: TrailDetailsInfo.mapPointValues(trailHead))
                    }
                    
                    // Trail End
                    if let trailEnd = navigationInfo.trailEnd {
                        trailEndValues.append(contentsOf: TrailDetailsInfo.mapPointValues(trailEnd))
                    }
                    
                    // Waypoints
                    navigationInfo.waypoints.forEach({ (waypoint) in
                        waypoints.append(TrailDetailsInfo.mapPointValues(waypoint))
                    })
                }
        ...
        static func mapPointValues(_ mapPoint: MapPoint) -> [(key: String, value: String)] {
            ...
            values.append(contentsOf: TrailDetailsInfo.locationValues(mapPoint.location))
